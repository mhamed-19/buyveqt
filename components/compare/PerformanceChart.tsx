    ...
    <ChartTooltipWrapper>
      ...
      {payload.map((p) => (
        <p key={p.dataKey} className="flex items-center gap-2 text-sm">
          <span className="w-2 h-2 rounded-full" style={{ backgroundColor: p.color }} />
          <span className="font-medium">{p.dataKey.replace(".TO", "")}</span>
          <span className="ml-auto tabular-nums">{formatDollars(p.value)}</span>
        </p>
      ))}
    </ChartTooltipWrapper>
  );
...
export default function PerformanceChart({ selectedFunds }: PerformanceChartProps) {
  ...
  const [rawData, setRawData] = useState<RawFundData[]>([]);
  ...
  const chartData = useMemo(() => {
    ...
    for (const { ticker, data } of rawData) {
      ...
      for (const point of data) {
        ...
        if (viewMode === "dollar") {
          normalized[point.date][ticker] = (point.close / startPrice) * 10000;
        } else {
  ...
  return (
    <div className="rounded-lg border border-[var(--color-border)] bg-white p-4 sm:p-5">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4">
        <div className="flex items-center gap-3">
          <h2 className="text-sm font-semibold text-[var(--color-text-secondary)]">
            {isDollar ? "Growth of $10,000" : "Normalized Performance (% change)"}
          </h2>
          ...
          <ResponsiveContainer width="100%" height={320}>
            <LineChart data={chartData}>
              ...
              <YAxis
                ...
                {...AXIS_PROPS}
                width={isDollar ? 70 : 50}
              />
              <Tooltip content={isDollar ? <DollarTooltip /> : <PercentTooltip />} />
              <Legend
      ...
      <p className="text-[11px] text-[var(--color-text-muted)] mt-2">
        Source: Yahoo Finance &middot;{" "}
        {isDollar
          ? "Hypothetical growth of $10,000 invested at start of period"
          : "Normalized to % change from start of period"}
      </p>
    </div>
  );
}
